#### Документация проекта kudab.ru

- [architecture.md](architecture.md) — архитектура и схемы
- [db-schema.md](db-schema.md) — описание структуры базы данных
- [database.dbml](source/database.dbml) — ER-диаграмма для визуализации
- [setup.md](setup.md) — быстрый старт и настройка окружения
- [api.md](api.md) — документация по API
- [bot.md](bot.md) — сценарии работы Telegram-бота
- [publisher.md](publisher.md) — автоматические публикации
- [frontend.md](frontend.md) — описание фронтенда (Nuxt.js)
- [roadmap.md](roadmap.md) — планы и задачи
- [business.md](business.md) — бизнес-требования, user stories
- [styleguide.md](styleguide.md) — стиль кода и best practices
- [migrations.md](migrations.md) — история изменений в БД

Каждый файл структурирован для быстрого понимания и работы.